...
def create_table(vdb: VDBHandler):
...
def test_insert_data(vdb: VDBHandler):
    data = {}
    data["database_name"] = "test_db"
    data["table_name"] = "test_table"
    data_id = str(uuid.uuid4())
    upsert_data = [[data_id], [np.random.normal(0, 0.05, 1536)], [
        "agent1"], ["test text"]]  # 分别代表table的各个字段, 依次输入, 可以一次性输入多个记录
    data["upsert_data"] = upsert_data
    vdb.upsert_Data(data)


def test_query_data(vdb: VDBHandler):
    data = {}
    data["database_name"] = "test_db"
    data["table_name"] = "test_table"
    ...
    # logging.info(rr)
    logging.info(rr[0])
    logging.info(rr[0][0].to_dict())


def test_delete_data(vdb: VDBHandler, delete_all=False, delete_id=None):
    data = {}
    data["database_name"] = "test_db"
    data["table_name"] = "test_table"
    if delete_all:
